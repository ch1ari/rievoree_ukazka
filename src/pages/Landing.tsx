      ...
      <div className="mx-auto max-w-5xl space-y-24 px-6 py-24 md:px-16">
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {VALUE.map((v, i) => (
              <motion.div key={v.k}
                ...
                transition={{ duration: 0.5, delay: i * 0.12, ease: "easeOut" }} whileHover={{ y: -6 }}
        ...
        <Reveal>
          ...
          <div className="mt-5 overflow-hidden rounded-3xl bg-card shadow-soft ring-1 ring-border">
            {ARCH.map((a, i) => (
              <div key={a.layer} className="group flex items-baseline gap-4 px-6 py-4 transition-colors not-last:border-b not-last:border-border hover:bg-secondary">
                <span className="font-mono text-xs text-accent transition-transform group-hover:scale-125">{String(i + 1).padStart(2, "0")}</span>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {STACK.map((s) => (
              <span key={s} className="rounded-full bg-secondary px-3 py-1 font-mono text-xs text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground">{s}</span>
            ))}
          </div>
        </Reveal>
        ...
        {/* 4 — Demo preview */}
        <Reveal>
          <motion.div whileHover={{ y: -4 }} className="overflow-hidden rounded-3xl bg-card shadow-soft ring-1 ring-border">
            <div className="flex items-center gap-2 border-b border-border px-6 py-4 font-mono text-xs text-muted-foreground">
              <span className="size-2 animate-pulse rounded-full bg-accent" /> 🔬 X-ray panel · live on every page
            </div>
            <div className="grid gap-px bg-border md:grid-cols-3">
              {["CALLS — timed at the fetch seam", "PIPELINE — Realtime event timeline", "RLS — same query, three identities"].map((t) => (
                <div key={t} className="bg-card p-6 font-mono text-xs text-muted-foreground transition-colors hover:bg-secondary">{t}</div>
              ))}
            </div>
